...
f_us <- fetchGDB(dsn ="D:/geodata/soils/gSSURGO_CONUS.gdb", WHERE = "areasymbol LIKE '%'")
mu_us <- get_mapunit_from_GDB(dsn = "D:/geodata/soils/gSSURGO_CONUS.gdb", WHERE = "muname LIKE '%'", stats = TRUE)
...
co_agg <- aqp::segment(aqp::horizons(f_us), intervals = c(0, 30, 100), hzdepcols = c("hzdept_r", "hzdepb_r")) %>%
  ...
  summarize(ec_r  = weighted.mean(ec_r,  w = hzdepb_r - hzdept_r, na.rm = TRUE),
            ph1to1h2o_r  = log10(weighted.mean(1/10^-ph1to1h2o_r,  w = hzdepb_r - hzdept_r, na.rm = TRUE)),
...
mu_agg_h <- co_agg %>%
  ...
  summarize(ec_r  = weighted.mean(ec_r, w = comppct_r, na.rm = TRUE),
            ph1to1h2o_r = log10(weighted.mean(1/10^-ph1to1h2o_r, w = comppct_r, na.rm = TRUE)),
...
mu_agg_h2 <- reshape(mu_agg_h,
                     ...
                     idvar     = "mukey",
                     timevar   = "segment_id",
                     v.names   = c("ec_r", "ph1to1h2o_r", "sar_r")
                     )
