...
#test model; no cv
ec030_rf_top25 <- ranger(y = log(train2$ec_ptf.000_030_cm +0.1), x = train2[idx], 
                    quantreg = TRUE, 
                    importance = "permutation",
                    respect.unordered.factors = NULL,
                    seed = 111 
...
#test model; no cv
ec100_rf_top25 <- ranger(y = log(train2$ec_ptf.030_100_cm +0.1), x = train2[idx], 
                    quantreg = TRUE, 
                    importance = "permutation",
                    respect.unordered.factors = NULL,
                    seed = 111 
...
idx <- which(names(train1) %in% 
               ...
               c(as.character(fs_b_stats$ec_ptf.000_030_cm$variable[13:43]),
               "statsgo_pmkind3"))) 

ec_rf_030 <- train(y = log(train1$ec_ptf.000_030_cm +0.1), x = train1[idx], 
...
idx <- which(names(train2) %in% 
              ...
              c(as.character(fs_b_stats$ec_ptf.030_100_cm$variable[19:43]), #leave out mlra
               "statsgo_pmkind3")))
...
save(train1, train2, ec_rf_030, ec_rf_100, 
     file = "K:/GSP/modeling/predict/ec_rf.RData")
...
ec_030_r <- predict(rs2, ec_rf_030$finalModel, fun = predfun, index = 1, progress = "text", overwrite = TRUE, filename = "K:/GSP/modeling/predict/ec_030_t.tif")
...
ec_100_r <- predict(rs3, ec_rf_100$finalModel, fun = predfun, index = 1, progress = "text", overwrite = TRUE, filename = "K:/GSP/modeling/predict/ec_100_t.tif")
