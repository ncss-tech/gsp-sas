...
s_wa <- aqp::segment(horizons(spc), intervals = c(0, 30, 100, 150), hzdepcols = c("hzn_top", "hzn_bot")) %>%
  ...
  summarize(ec_wa = weighted.mean(electrical_conductivity_satx, w = hzn_mid, na.rm = TRUE),
            ph_wa = log10(weighted.mean(1/10^-ph_saturated_paste, w = hzn_mid, na.rm = TRUE)),
            sar_wa = weighted.mean(sodium_absorption_ratio, w = hzn_mid, na.rm = TRUE)
            ) %>%
  ungroup() %>%
