...
idx <- which(names(train2) %in% fs_b_stats$ph_ptf.030_100_cm$variable[1:25])
...
idx <- which(
  names(rs) %in% ph_rf_t25$forest$independent.variable.names
  )
rs2 <- rs[[idx]]
ph_030_r <- predict(rs2, ph_rf_t25, fun = predfun, index = 1, progress = "text", overwrite = TRUE, filename = "D:geodata/project_data/gsp-sas/ph_030_qrf_mlra_pm_NULL.tif")
