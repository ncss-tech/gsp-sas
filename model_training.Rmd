...
vars <- c("ec_ptf.000_030_cm",
          isric_vars, 
          ssurgo_vars[ssurgo_vars %in% c("gnatsgo_pmkind", "mlra_rs", "ph30_rs_average", "sar30_rs_average")]
          )
train2 <- train[vars]

idx <- sapply(train2, is.numeric)
idx <- which(sapply(train2[idx], function(x) all(x + 0.1 > 0, na.rm = TRUE) & median(x, na.rm = TRUE) > 0))
train3 <- na.exclude(train2)
train3[idx][1] <- lapply(train3[idx][1], function(x) {
  t <- car::powerTransform(x + 0.1)
  y <- x ^ t$lambda
})

set.seed(111)
ec_rt3 <- train(y = log(train5[[1]] + 0.1), x = train5[-1], 
               ...
               # trControl = trainControl(method = "cv", number = 10, repeats = 5, returnResamp = "all", savePredictions = TRUE, search = "random", verboseIter = FALSE), 
                trControl = trainControl(method = "none"), 
                tuneGrid = expand.grid(.mtry = 20/ 3, # round(ncol(train2[-1]) / 3), 
                                       .splitrule = "variance", .min.node.size = c(5))
)
plot(varImp(ec_rt2))
...
test <- within(test, {
  names.test. = ifelse(grepl("mlra", names.test.), "mlra_rs", names.test.)
  ssurgo_pm_lu3_rs_mode = ifelse(grepl("ssurgo_pm", names.test.), "ssurgo_pm_lu3_rs_mode", names.test.)
  nlcd_2016_conus = ifelse(grepl("nlcd", names.test.), "nlcd_2016_conus", names.test.)
})
test = test[!duplicated(test$names.test.), ]
train5 <- cbind(train2[1], train2[names(train2) %in% test$names.test.[1:20]])
train5 <- na.exclude(train5)

y = log(na.exclude(train2)$ec_ptf.000_030_cm + 0.1)
test_ex <- explain(ec_rt2$finalModel, data = na.exclude(train2)[-1], y = y)
test_p  <- model_performance(test_ex)
test_md <- model_diagnostics(test_ex)
plot(test_md, variable = "y", yvariable = "y_hat") + geom_abline(intercept = 0, slope = 1)
plot(test_md, variable = "y", yvariable = "residuals")
