...
train30 <- na.exclude(train[vars])

#transform independent variable
hist(train30$ec_ptf.000_030_cm)
ec30t = log(train30$ec_ptf.000_030_cm + 0.1)
hist(ec30t)
...
idx <- which(names(train30) %in% fs_b_stats$ec_ptf.000_030_cm$variable[fs_b_stats$ec_ptf.000_030_cm$decision == "Confirmed"])
#test model; no cv
ec030_rf_all <- ranger(y = ec30t, x = train30[idx], 
                    quantreg = TRUE, 
...
idx <- which(names(train30) %in% fs_b_stats$ec_ptf.000_030_cm$variable[1:44])
#test model; no cv
ec030_rf_half <- ranger(y = ec30t, x = train30[idx], 
                    quantreg = TRUE, 
                    importance = "permutation",
                    respect.unordered.factors = NULL,
                    seed = 111 
...
idx <- which(names(train30) %in% fs_b_stats$ec_ptf.000_030_cm$variable[1:18])
#test model; no cv
ec030_rf_top <- ranger(y = ec30t, x = train30[idx], 
                    quantreg = TRUE, 
                    importance = "permutation",
                    respect.unordered.factors = NULL,
                    seed = 111 
...
#(4) top 25 variables
idx <- which(names(train30) %in% fs_b_stats$ec_ptf.000_030_cm$variable[1:25])
#test model; no cv
ec030_rf_top25 <- ranger(y = ec30t, x = train30[idx], 
                    quantreg = TRUE, 
                    importance = "permutation",
                    respect.unordered.factors = NULL,
                    seed = 111 
...
train100 <- na.exclude(train[vars])

#transform independent variable
hist(train100$ec_ptf.030_100_cm)
ec100t = log(train100$ec_ptf.030_100_cm + 0.1)
hist(ec100t)

#(1) all confirmed variables
idx <- which(names(train100) %in% fs_b_stats$ec_ptf.030_100_cm$variable[fs_b_stats$ec_ptf.030_100_cm$decision == "Confirmed"])
#test model; no cv
ec100_rf_all <- ranger(y = ec100t, x = train100[idx], 
                    quantreg = TRUE, 
                    importance = "permutation",
                    respect.unordered.factors = NULL,
                    seed = 111 
...
idx <- which(names(train100) %in% fs_b_stats$ec_ptf.030_100_cm$variable[1:43])
#test model; no cv
ec100_rf_half <- ranger(y = ec100t, x = train100[idx], 
                    quantreg = TRUE, 
                    importance = "permutation",
                    respect.unordered.factors = NULL,
                    seed = 111 
...
#(3) variables based on boruta plot 
idx <- which(names(train100) %in% fs_b_stats$ec_ptf.030_100_cm$variable[1:18])
#test model; no cv
ec100_rf_top <- ranger(y = ec100t, x = train100[idx], 
                    quantreg = TRUE, 
                    importance = "permutation",
                    respect.unordered.factors = NULL,
                    seed = 111 
...
#(4) top 25 variables
idx <- which(names(train100) %in% fs_b_stats$ec_ptf.030_100_cm$variable[1:25])
#test model; no cv
ec100_rf_top25 <- ranger(y = ec100t, x = train100[idx], 
                    quantreg = TRUE, 
                    importance = "permutation",
                    respect.unordered.factors = NULL,
                    seed = 111 
...
#qrf w/cv
idx <- which(names(train30) %in% 
               c(as.character(fs_b_stats$ec_ptf.000_030_cm$variable[1:29]), 
               c(as.character(fs_b_stats$ec_ptf.000_030_cm$variable[31:44]),
               "statsgo_pmkind3"))) 

ec_rf_030 <- train(y = ec30t, x = train30[idx], 
...
#qrf w/cv
idx <- which(names(train100) %in% 
               c(as.character(fs_b_stats$ec_ptf.030_100_cm$variable[1:39]),   
              c(as.character(fs_b_stats$ec_ptf.030_100_cm$variable[41:43]), 
               "statsgo_pmkind3")))

ec_rf_100 <- train(y = ec100t, x = train100[idx], 
               method = "ranger", quantreg = TRUE, 
               importance = "permutation", 
               respect.unordered.factors = NULL, #needed if mlra is in the variables
               seed = 111,
...
# variable importance plots
vip::vip(ec_rf_100, 45)

print(ec_rf_030)
print(ec_rf_100)

save(train30, train100, ec_rf_030, ec_rf_100,file = "K:/GSP/modeling/predict/ec_rf.RData")
...
ec_030_r <- predict(rs2, ec_rf_030$finalModel, fun = predfun, index = 1, progress = "text", overwrite = TRUE, filename = "K:/GSP/modeling/predict/ec030t.tif")
...
ec_100_r <- predict(rs3, ec_rf_100$finalModel, fun = predfun, index = 1, progress = "text", overwrite = TRUE, filename = "K:/GSP/modeling/predict/ec100t.tif")

##back-transform values
ec_030_r_bt <- calc(ec_030_r, function(x) exp(x) - 0.1)
writeRaster(ec_030_r_bt, "K:/GSP/modeling/predict/ec030bt.tif", overwrite = TRUE)

ec_100_r_bt <- calc(ec_100_r, function(x) exp(x) - 0.1)
writeRaster(ec_100_r_bt, "K:/GSP/modeling/predict/ec100bt.tif", overwrite = TRUE)


##fix nodata holes and water as NA in final predictions
library(raster)

ec_030_r <- raster("K:/GSP/modeling/predict/ec030bt.tif")
ec_100_r <- raster("K:/GSP/modeling/predict/ec100bt.tif")
...
ec_030_r2 <- merge(ec_030_r, ec_030_r0, overlap = TRUE, filename = "K:/GSP/modeling/predict/final_maps/ec030bt_fill.tif", progress = "text", overwrite = TRUE)
ec_100_r2 <- merge(ec_100_r, ec_100_r0, overlap = TRUE, filename = "K:/GSP/modeling/predict/final_maps/ec100bt_fill.tif", progress = "text", overwrite = TRUE)
...
mask(ec_030_r2, lc, filename = "K:/GSP/modeling/predict/final_maps/ec030bt_fill_mask.tif", overwrite = TRUE)

mask(ec_100_r2, lc, filename = "K:/GSP/modeling/predict/final_maps/ec100bt_fill_mask.tif", overwrite = TRUE)

#evaluate results
ec_30f <- raster("K:/GSP/modeling/predict/final_maps/ec030bt_fill_mask.tif")
na30 <- is.na(ec_30f) 
summary(na30@data@values==TRUE)
plot(na30)

ec_100f <- raster("K:/GSP/modeling/predict/final_maps/ec100bt_fill_mask.tif")
na100 <- is.na(ec_100f) 
...
vars <- c(ec_030 = "K:/GSP/modeling/predict/final_maps/ec030bt_fill_mask.tif",
          ec_100 = "K:/GSP/modeling/predict/final_maps/ec100bt_fill_mask.tif")
rs4 <- stack(vars)
