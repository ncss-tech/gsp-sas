...
co_agg <- aqp::segment(aqp::horizons(f_us), intervals = c(0, 100), hzdepcols = c("hzdept_r", "hzdepb_r")) %>%
  group_by(cokey) %>%
  summarize(ec_r = weighted.mean(ec_r, w = hzdepb_r - hzdept_r)) %>%
  ungroup() %>%
  right_join(aqp::site(f_us), by = "cokey") 
